/*
 *  Methods for "Blowfish" cipher algorithm
...
static uint32_t cm_bf_feist(const cm_bf_ctxt_t *ctxt, uint32_t x)
{   uint32_t a, b, c, d, y;
...
{   uint32_t Xl, Xr, temp;
    ...
    for (round=0; round<CM_BF_nrounds; ++round) {
        Xl = Xl ^ ctxt->p[round];
        Xr = cm_bf_feist(ctxt, Xl) ^ Xr;
    ...
    Xr = Xr ^ ctxt->p[CM_BF_nrounds];
    Xl = Xl ^ ctxt->p[CM_BF_nrounds + 1];
...
{   uint32_t Xl, Xr, temp;
    ...
    for (round=CM_BF_nrounds+1; round>1; --round) {
        Xl = Xl ^ ctxt->p[round];
        Xr = cm_bf_feist(ctxt, Xl) ^ Xr;
...
{   cm_bf_ctxt_t *ctxt;
    ...
    ctxt = (cm_bf_ctxt_t*)malloc(sizeof(cm_bf_ctxt_t));
    memcpy((void*)ctxt->p, (const void*)bf_p, sizeof(bf_p));
    memcpy((void*)ctxt->sbox, (const void*)bf_sbox, sizeof(bf_sbox));

    j = 0;
    for (i=0; i<CM_BF_nrounds+2; ++i) {
        data = 0;
        for (k=0; k<4; ++k) {
            data = (data << 8) | (uint32_t)key[j];
            if (++j >= keybytes) j = 0;
        }
        ctxt->p[i] =ctxt->p[i] ^ data;
    }

    datal = datar = 0;

    for (i=0; i<CM_BF_nrounds+2; i+= 2) {
        cm_bf_encipher(ctxt, &datal, &datar);
...
void cm_bf_free(cm_bf_ctxt_t *ctxt)
{
    ...
    memset((void*)ctxt->p, 0, sizeof(bf_p));
    memset((void*)ctxt->sbox, 0, sizeof(bf_sbox));
...
int bf_test_blowfish()
    /* Check Blowfish encryption against known test-vectors */
{   struct tvec {
        const char *key;
        uint32_t in0, in1;
        uint32_t out0, out1; }
    tvecs[] = {
        { "abcdefgh", 0x01234567, 0x89abcdef,
                    0x89fc0a5b, 0x074df537, },
        { "ABCDwxyz", 0x12345678, 0x9abcdef0,
                    0x061dab8f, 0x33a16a10},
        { "alphabetti spaghetti", 0xaa55aa55, 0x55aa55aa,
                    0x927079bb, 0xb275bad0 },
        { "Blowfish algorithm", 0x5555aaaa, 0x01234567,
                    0x731ae03a, 0xda77789e },
        { NULL, 0, 0 } };
    cm_bf_ctxt_t *ctxt;
    uint32_t vals[2];
    unsigned idx;

    CM_TEST_START("Internal Blowfish");

    idx = 0;
    while (tvecs[idx].key != NULL) {
        ctxt = cm_bf_init((uint8_t*)tvecs[idx].key,
                            strlen(tvecs[idx].key));
        vals[0] = tvecs[idx].in0;
        vals[1] = tvecs[idx].in1;

        cm_bf_encipher(ctxt, vals, vals+1);
        CM_ASSERT_EQUAL(tvecs[idx].out0, vals[0]);
        CM_ASSERT_EQUAL(tvecs[idx].out1, vals[1]);

        cm_bf_decipher(ctxt, vals, vals+1);
        CM_ASSERT_EQUAL(tvecs[idx].in0, vals[0]);
        CM_ASSERT_EQUAL(tvecs[idx].in1, vals[1]);
